...
public class GlobalExceptionHandler {
  ...
  @ExceptionHandler(CRDataNotFoundException.class)
  public ResponseEntity<String> handleCRDataNotFoundException(CRDataNotFoundException ex) {
    return ResponseEntity.status(HttpStatus.NOT_FOUND)
        .body(
            Optional.ofNullable(ex.getMessage()).orElse("Unexpected exception from SectionModule"));
  }

  @ExceptionHandler(CRValidationException.class)
  public ResponseEntity<String> handleCRValidationException(CRValidationException ex) {
    return ResponseEntity.status(HttpStatus.BAD_REQUEST)
        .body(
            Optional.ofNullable(ex.getMessage()).orElse("Unexpected exception from SectionModule"));
  }
   ...
   * @return A String containing the error message with HTTP 400 (Bad Request) status.
   */
   ...
   *
   * @param ex The exception.
   * @return A String containing the error message with HTTP 404 (Not Found) status.
   */
